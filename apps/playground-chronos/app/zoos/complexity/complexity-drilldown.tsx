...
export function ComplexityDrilldown({ files }: { files: FileEntry[] }) {
  ...
  const sorted = useMemo(() => {
    const result = files.filter((f) => f.path.toLowerCase().includes(filter.toLowerCase()));
      ...
      <OrnateFrame pillar="chronos" variant="panel">
        {sorted.slice(0, 100).map((f) => (
          <div key={f.path} style={{ marginBottom: '0.25rem' }}>
            ...
            >
              <span style={{ fontSize: '0.75rem', opacity: 0.5 }}>{expanded === f.path ? '▼' : '▶'}</span>
              <span
                style={{
                  flex: 1,
              ...
              <span style={{ fontSize: '0.75rem', opacity: 0.5 }}>{f.functions.length} fn</span>
              <span style={{ fontSize: '0.75rem', opacity: 0.5 }}>{f.loc} LOC</span>
              <span
                style={{
                  fontWeight: 600,
                  fontSize: '0.85rem',
                ...
                {f.functions
                  ...
                  .map((fn, i) => (
                    ...
                    >
                      ...
                      <span style={{ fontSize: '0.7rem', opacity: 0.5 }}>{fn.complexity.loc} LOC</span>
                      <span
                        style={{
                          fontWeight: 600,
                          fontSize: '0.8rem',

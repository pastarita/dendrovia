...
export function FilesTable({ files }: { files: FileEntry[] }) {
  ...
  const sorted = useMemo(() => {
    const result = files.filter(
      (f) =>
        f.path.toLowerCase().includes(filter.toLowerCase()) || f.language.toLowerCase().includes(filter.toLowerCase()),
    );
          ...
          <tbody>
            {sorted.slice(0, 200).map((f) => (
              <tr key={f.path} style={{ borderBottom: '1px solid #1a1a1a' }}>
                ...
                <td style={{ padding: '0.4rem 0.5rem', textAlign: 'right', opacity: 0.7 }}>{f.loc}</td>
              </tr>
            ))}
          </tbody>
        </table>

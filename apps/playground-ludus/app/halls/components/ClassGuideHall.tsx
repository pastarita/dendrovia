...
import { BASE_STATS, computeStatsAtLevel, createCharacter, GROWTH_RATES, getSpell } from '@dendrovia/ludus';
...
const CLASS_META: Record<string, { label: string; role: string; archetype: string; color: string }> = {
  tank: {
    label: 'Tank',
    role: 'Absorb damage, protect team',
    archetype: 'Infrastructure Developer — DevOps, Platform Maintainer',
    color: '#3B82F6',
  },
  healer: {
    label: 'Healer',
    role: 'Restore HP, remove debuffs',
    archetype: 'Bug Fixer — Debugger, QA Engineer',
    color: '#22C55E',
  },
  dps: {
    label: 'DPS',
    role: 'Deal damage, eliminate threats',
    archetype: 'Feature Developer — Full-stack Dev, Feature Lead',
    color: '#EF4444',
  },
};
...
export default function ClassGuideHall(): React.JSX.Element {
  const classData = useMemo(() => {
    return CLASSES.map((cls) => {
      ...
      const spellDetails = char30.spells
        .map((id) => {
  ...
  return (
    <div style={{ display: 'grid', gap: '1.5rem' }}>
      ...
      {classData.map((cd) => (
        ...
        >
          <div style={{ fontWeight: 700, fontSize: '1.1rem', color: cd.meta.color, marginBottom: '0.25rem' }}>
            {cd.meta.label}
          </div>
          ...
          {/* Spell List */}
          <div style={{ fontSize: '0.7rem', opacity: 0.4, marginBottom: '0.35rem' }}>
            Spells ({cd.spellDetails.length})
          </div>
            ...
            {cd.spellDetails.map((spell: any) => (
              ...
              >
                ...
                >
                  ...
                  <span style={{ fontSize: '0.65rem', opacity: 0.4, fontFamily: 'var(--font-geist-mono)' }}>
                    {spell.element}
                  </span>
                </div>

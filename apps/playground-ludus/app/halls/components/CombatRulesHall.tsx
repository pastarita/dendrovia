  ...
  return (
    <div>
      ...
      <div style={sectionStyle}>
        ...
        >
          <span
            style={{ padding: '0.3rem 0.6rem', background: '#1a3a1a', border: '1px solid #333', borderRadius: '4px' }}
          >
            PLAYER_TURN
          </span>
          <span style={{ opacity: 0.4 }}>&rarr;</span>
          <span
            style={{ padding: '0.3rem 0.6rem', background: '#3a1a1a', border: '1px solid #333', borderRadius: '4px' }}
          >
            ENEMY_TURN
          </span>
          <span style={{ opacity: 0.4 }}>&rarr;</span>
        ...
        <div style={{ fontSize: '0.8rem', opacity: 0.6, marginTop: '0.5rem' }}>
          Each turn: player acts first, then all enemies act. Status effects tick at the start of each entity&apos;s
          turn. Victory when all enemies reach 0 HP. Defeat when the player reaches 0 HP.
        </div>
      </div>
          ...
          {[
            {
              name: 'Attack',
              key: 'ATTACK',
              desc: 'Basic physical attack against a target. No mana cost. Uses ATK vs DEF. Element: none.',
            },
            {
              name: 'Cast Spell',
              key: 'CAST_SPELL',
              desc: 'Use a learned spell. Costs mana. Subject to cooldown. Uses spell power + ATK vs DEF with element multipliers.',
            },
            {
              name: 'Defend',
              key: 'DEFEND',
              desc: 'Raise guard. Grants +5 DEF for 1 turn (defense-up status effect). No mana cost. No cooldown.',
            },
            {
              name: 'Use Item',
              key: 'USE_ITEM',
              desc: 'Consume an item from inventory. Heals HP/mana, buffs stats, or cleanses debuffs. No cooldown.',
            },
          ].map((action) => (
            <div
              key={action.key}
            ...
            >
              ...
              >
                ...
                <span style={{ fontFamily: 'var(--font-geist-mono)', fontSize: '0.65rem', opacity: 0.4 }}>
                  {action.key}
                </span>
              </div>
      ...
      <div style={sectionStyle}>
        ...
        <ul style={listStyle}>
          <li>
            Each spell has a{' '}
            <code style={{ fontFamily: 'var(--font-geist-mono)', color: 'var(--pillar-accent)' }}>cooldown</code> value
            (in turns).
          </li>
          <li>After casting, the spell cannot be reused for that many turns.</li>
          <li>
            Cooldowns are tracked per-spell in the character&apos;s{' '}
            <code style={{ fontFamily: 'var(--font-geist-mono)', color: 'var(--pillar-accent)' }}>cooldowns</code> map.
          </li>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ fontSize: '0.8rem', opacity: 0.5, marginTop: '0.75rem' }}>
          <strong>Stacking:</strong> Effects do not stack. Re-applying refreshes duration (or takes the highest value
          for stat mods).
          <br />
          <strong>Tick order:</strong> Effects tick at the start of the affected entity&apos;s turn, then duration
          decrements. Expired effects are removed.
        </div>
      </div>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ fontSize: '0.85rem', opacity: 0.7 }}>
          Using Defend applies a{' '}
          <code style={{ fontFamily: 'var(--font-geist-mono)', color: 'var(--pillar-accent)' }}>defense-up</code> status
          effect with value +5 and duration 1 turn. This effectively increases the character&apos;s DEF stat by 5 for
          the enemy&apos;s next attack, then expires.
        </div>
      </div>
    </div>
  );

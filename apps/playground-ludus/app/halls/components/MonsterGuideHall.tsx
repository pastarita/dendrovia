...
const TYPE_META: Record<string, { label: string; description: string; behavior: string; color: string }> = {
  'null-pointer': {
    ...
    description: 'The most common bug in the codebase. Occurs when code tries to access a property of null or undefined.',
  ...
  return (
    <div style={{ display: 'grid', gap: '1.5rem' }}>
      ...
      <div style={{ padding: '1rem', border: '1px solid #222', borderRadius: '8px', background: '#111' }}>
        <div style={{ fontSize: '0.7rem', opacity: 0.4, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.35rem' }}>XP Reward Formula</div>
        <div style={{ fontFamily: 'var(--font-geist-mono)', fontSize: '0.85rem' }}>
          xpReward = floor(25 * severity^2 * (1 + 0.05 * complexity))
        </div>
        <div style={{ display: 'flex', gap: '0.75rem', fontSize: '0.75rem', fontFamily: 'var(--font-geist-mono)', opacity: 0.5, marginTop: '0.35rem' }}>
          {SEVERITIES.map(s => (
            <span key={s}>S{s}: {xpRewardForMonster(s, 0)} XP</span>
          ))}
        </div>
      </div>
      ...
      {monsterData.map(md => (
        ...
        >
          ...
          <div style={{ marginBottom: '0.5rem' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              {md.spellDetails.map((spell: any) => (
                <div key={spell.id} style={{ padding: '0.35rem 0.5rem', border: '1px solid #222', borderRadius: '4px', background: '#0a0a0a', fontSize: '0.75rem' }}>
                  <span style={{ fontWeight: 600 }}>{spell.name}</span>
                  <span style={{ opacity: 0.5, marginLeft: '0.35rem' }}>{spell.effect.type} {spell.effect.value}</span>
                </div>
              ))}
            </div>
          </div>
          ...
          {/* Loot Table */}
          <div>
            <div style={{ fontSize: '0.7rem', opacity: 0.4, marginBottom: '0.25rem' }}>Loot Table (at max severity)</div>

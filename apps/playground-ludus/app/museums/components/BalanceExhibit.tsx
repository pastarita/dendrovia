...
export default function BalanceExhibit(): React.JSX.Element {
  ...
  const [matchupResult, setMatchupResult] = useState<Array<{
    config: string;
    winRate: number;
    avgTurns: number;
    flag: string;
  ...
  return (
    <div>
      ...
      <OrnateFrame
        pillar="ludus"
        variant="panel"
        style={{ background: '#111', marginBottom: '1.5rem', overflowX: 'auto' }}
      >
        <div
          style={{
            fontSize: '0.7rem',
        ...
        <table style={{ borderCollapse: 'collapse', width: '100%' }}>
          ...
          <tbody>
            {KEY_VALUES.map((kv) => (
              <tr key={kv.path}>
                <td style={{ padding: '0.25rem 0.5rem', fontSize: '0.8rem', borderBottom: '1px solid #1a1a1a' }}>
                  {kv.label}
                </td>
                {CONFIGS.map((c) => {
      ...
      <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'flex-end', flexWrap: 'wrap', marginBottom: '1rem' }}>
        <div>
          ...
          <select
            value={playerClass}
            onChange={(e) => setPlayerClass(e.target.value as CharacterClass)}
            style={selectStyle}
          >
            {CLASSES.map((c) => (
        ...
        <OrnateFrame pillar="ludus" variant="panel" style={{ background: '#111', marginBottom: '1.5rem' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem' }}>
            {matchupResult.map((r) => (
              <div
                key={r.config}
                style={{ padding: '0.75rem', border: '1px solid #222', borderRadius: '6px', background: '#1a1a1a' }}
              >
                ...
                <div style={{ fontFamily: 'var(--font-geist-mono)', fontSize: '0.8rem' }}>
                  <div>
                    Win Rate: <span style={{ fontWeight: 700 }}>{(r.winRate * 100).toFixed(1)}%</span>
                  </div>
      ...
      <OrnateFrame pillar="ludus" variant="panel" style={{ background: '#111', marginBottom: '1rem' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.5rem', fontSize: '0.8rem' }}>
          <div>
            <span style={{ color: FLAG_COLORS.ok, fontWeight: 700 }}>ok</span> — Win rate 30-80%
          </div>
          <div>
            <span style={{ color: FLAG_COLORS['too-easy'], fontWeight: 700 }}>too-easy</span> — Win rate &gt;80%
          </div>
          <div>
            <span style={{ color: FLAG_COLORS['too-hard'], fontWeight: 700 }}>too-hard</span> — Win rate &lt;30%
          </div>
          <div>
            <span style={{ color: FLAG_COLORS['draw-heavy'], fontWeight: 700 }}>draw-heavy</span> — Draw rate &gt;10%
          </div>
        </div>
      </OrnateFrame>

      <div style={{ fontSize: '0.8rem', opacity: 0.5 }}>
        For full interactive balance simulation, visit{' '}
        <a href="/gyms/balance" style={{ color: 'var(--pillar-accent)' }}>
          /gyms/balance
        </a>
      </div>
    </div>

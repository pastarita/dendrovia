  ...
  return (
    <div>
      ...
      >
        ...
        >
          <div>
            <span style={{ opacity: 0.5 }}>Raw Power:</span> {calc.rawPower}
          </div>
          <div>
            <span style={{ opacity: 0.5 }}>DEF Reduction:</span> {(calc.defReduction * 100).toFixed(1)}%
          </div>
          <div>
            <span style={{ opacity: 0.5 }}>Base Damage:</span> {calc.baseDamage.toFixed(1)}
          </div>
          <div>
            <span style={{ opacity: 0.5 }}>Element Mult:</span>{' '}
            <span style={{ color: multColor(calc.elementMult) }}>{calc.elementMult}x</span>
          </div>
          <div>
            <span style={{ opacity: 0.5 }}>Crit Chance:</span> {(calc.critChance * 100).toFixed(1)}%
          </div>
          <div>
            <span style={{ opacity: 0.5 }}>Variance:</span> [{VARIANCE_MIN}, {VARIANCE_MIN + VARIANCE_RANGE}]
          </div>
          <div>
            <span style={{ opacity: 0.5 }}>Damage Range:</span>{' '}
            <span style={{ color: 'var(--pillar-accent)' }}>
              {calc.minDmg} — {calc.maxDmg}
            </span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        >
          <div>
            <span style={{ opacity: 0.5 }}>DEFENSE_CONSTANT:</span> {DEFENSE_CONSTANT}
          </div>
          <div>
            <span style={{ opacity: 0.5 }}>BASE_CRIT_CHANCE:</span> {BASE_CRIT_CHANCE}
          </div>
          <div>
            <span style={{ opacity: 0.5 }}>CRIT_MULTIPLIER:</span> {CRIT_MULTIPLIER}x
          </div>
          <div>
            <span style={{ opacity: 0.5 }}>MAX_CRIT_CHANCE:</span> {MAX_CRIT_CHANCE}
          </div>
          <div>
            <span style={{ opacity: 0.5 }}>CRIT_PER_SPEED:</span> {CRIT_PER_SPEED}
          </div>
          <div>
            <span style={{ opacity: 0.5 }}>VARIANCE:</span> [{VARIANCE_MIN},{' '}
            {(VARIANCE_MIN + VARIANCE_RANGE).toFixed(2)}]
          </div>
        </div>
      </div>
        ...
        <div style={{ overflowX: 'auto' }}>
          <table style={{ borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                <th
                  style={{ padding: '0.4rem 0.6rem', borderBottom: '1px solid #333', fontSize: '0.7rem', opacity: 0.5 }}
                >
                  ATK \ DEF
                </th>
                {ELEMENTS.map((el) => (

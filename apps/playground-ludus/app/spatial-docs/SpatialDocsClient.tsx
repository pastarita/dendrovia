...
const USER_ACTIONS = EVENT_CATALOG.filter(e => ['SPELL_CAST', 'ITEM_USED'].includes(e.constant));

const MODULES = [
  { module: 'CharacterSystem', file: 'character/CharacterSystem.ts', exports: 'createCharacter, computeStatsAtLevel, gainExperience, BASE_STATS, GROWTH_RATES', desc: 'Character factory, stat computation, XP/level progression' },
  { module: 'TurnBasedEngine', file: 'combat/TurnBasedEngine.ts', exports: 'initBattle, executeTurn, getAvailableActions', desc: 'Pure reducer for turn-based combat resolution' },
  { module: 'CombatMath', file: 'combat/CombatMath.ts', exports: 'calculateDamage, calculateHealing, calculateShield, ELEMENT_TABLE', desc: 'Damage/heal/shield formulas and element effectiveness' },
  { module: 'SpellFactory', file: 'spell/SpellFactory.ts', exports: 'generateSpell, getAllSpells, getSpell', desc: 'Spell registry and symbol-driven procedural generation' },
  { module: 'MonsterFactory', file: 'combat/MonsterFactory.ts', exports: 'createMonster, generateBoss, generateMiniboss, generateBugMonster', desc: 'Monster creation with type/severity/complexity scaling' },
  { module: 'StatusEffects', file: 'combat/StatusEffects.ts', exports: 'tickStatusEffects, absorbDamage, createStatusEffect', desc: 'Per-turn effect processing, shield absorption, stat modifiers' },
  { module: 'EnemyAI', file: 'combat/EnemyAI.ts', exports: 'chooseEnemyAction, resolveEnemySpell', desc: 'Type-specific AI behavior for each bug type + boss phases' },
  { module: 'QuestGenerator', file: 'quest/QuestGenerator.ts', exports: 'generateQuestGraph, generateBugHuntQuests, generateArchaeologyQuests', desc: 'Converts CHRONOS commits/files into quest DAGs' },
  { module: 'EncounterSystem', file: 'encounter/EncounterSystem.ts', exports: 'checkEncounter, scanAllEncounters, getEncounterDensity', desc: 'Spatial navigation triggers for combat encounters' },
  { module: 'InventorySystem', file: 'inventory/InventorySystem.ts', exports: 'getAllItems, useItem, resolveLoot, addItem', desc: 'Item registry, usage effects, loot resolution' },
  { module: 'ProgressionSystem', file: 'progression/ProgressionSystem.ts', exports: 'resolveBattleRewards, applyBattleRewards, createBattleStatistics', desc: 'Post-combat XP/loot pipeline and statistics tracking' },
  { module: 'SimulationHarness', file: 'simulation/SimulationHarness.ts', exports: 'simulateBattle, simulateMatchup, runFullSimulation', desc: 'Monte Carlo balance testing (1000+ trials per matchup)' },
  { module: 'BalanceConfig', file: 'config/BalanceConfig.ts', exports: 'DEFAULT_BALANCE_CONFIG, EASY_CONFIG, HARD_CONFIG', desc: 'All tuning knobs: damage, XP, monster scaling, encounter rates' },
  { module: 'SeededRandom', file: 'utils/SeededRandom.ts', exports: 'createRngState, rngNext, rngRange, rngChance', desc: 'sfc32 PRNG for deterministic replay (zero Math.random())' },
  { module: 'SaveSystem', file: 'save/SaveSystem.ts', exports: 'saveGame, loadGame, createSaveSlot', desc: 'Game state serialization and persistence' },
  { module: 'EventWiring', file: 'integration/EventWiring.ts', exports: 'wireLudusEvents', desc: 'Connects LUDUS systems to the shared EventBus' },
];
    ...
    <div>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ fontSize: '0.85rem', opacity: 0.6, marginBottom: '0.75rem' }}>
          All inter-pillar communication flows through <code style={{ fontFamily: 'var(--font-geist-mono)', color: 'var(--pillar-accent)' }}>GameEvents</code> constants
          on the shared <code style={{ fontFamily: 'var(--font-geist-mono)', color: 'var(--pillar-accent)' }}>EventBus</code>.
          LUDUS listens for spatial input events and emits combat/progression output events.
        </div>
            ...
            <tbody>
              {EVENT_CATALOG.map(ev => (
                <tr key={ev.constant}>
                  ...
                  <td style={tdStyle}>
                    <span style={{ color: DIRECTION_COLORS[ev.direction], fontSize: '0.7rem', fontWeight: 700 }}>{ev.direction}</span>
                  </td>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ fontSize: '0.85rem', opacity: 0.6, marginBottom: '0.75rem' }}>
          Emitted by ARCHITECTUS when the player navigates the 3D code world. LUDUS listens to these to trigger encounters and quest checks.
        </div>
        <div style={{ display: 'grid', gap: '0.5rem' }}>
          {SPATIAL_INPUTS.map(ev => (
            <div key={ev.constant} style={{ padding: '0.75rem', border: '1px solid #222', borderRadius: '6px', background: '#0a0a0a' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.15rem' }}>
                <span style={{ fontFamily: 'var(--font-geist-mono)', fontWeight: 700, fontSize: '0.85rem' }}>{ev.constant}</span>
                <span style={{ fontFamily: 'var(--font-geist-mono)', fontSize: '0.7rem', opacity: 0.4 }}>&quot;{ev.value}&quot;</span>
              </div>
              <div style={{ fontSize: '0.8rem', opacity: 0.6, marginBottom: '0.25rem' }}>{ev.description}</div>
              <div style={{ fontFamily: 'var(--font-geist-mono)', fontSize: '0.7rem', opacity: 0.4 }}>Payload: {ev.payload}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ fontSize: '0.85rem', opacity: 0.6, marginBottom: '0.75rem' }}>
          Emitted by LUDUS during combat. OCULUS listens for HUD updates; ARCHITECTUS listens for visual feedback (damage numbers, spell effects).
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '0.5rem' }}>
          {COMBAT_OUTPUTS.map(ev => (
            <div key={ev.constant} style={{ padding: '0.5rem 0.75rem', border: '1px solid #222', borderRadius: '6px', background: '#0a0a0a' }}>
              <div style={{ fontFamily: 'var(--font-geist-mono)', fontWeight: 600, fontSize: '0.8rem', marginBottom: '0.1rem' }}>{ev.constant}</div>
      ...
      <div style={sectionStyle}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem', fontFamily: 'var(--font-geist-mono)', fontSize: '0.8rem' }}>
          <div style={{ padding: '0.75rem', border: '1px solid #3B82F6', borderRadius: '6px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '0.7rem', opacity: 0.4, marginTop: '0.25rem' }}>player:moved, node:clicked, collision:detected, branch:entered</div>
          </div>
          <div style={{ padding: '0.75rem', border: '1px solid var(--pillar-accent)', borderRadius: '6px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '0.7rem', opacity: 0.4, marginTop: '0.25rem' }}>encounter check &rarr; combat &rarr; rewards &rarr; progression</div>
          </div>
          <div style={{ padding: '0.75rem', border: '1px solid #22C55E', borderRadius: '6px', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: '0.7rem', opacity: 0.4, marginTop: '0.25rem' }}>health bars, combat HUD, quest log, victory screen</div>
          </div>
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between', padding: '0.5rem 1rem', fontSize: '0.7rem', opacity: 0.3, marginTop: '0.5rem' }}>

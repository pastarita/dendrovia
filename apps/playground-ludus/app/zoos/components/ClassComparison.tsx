      ...
      {data.map(({ cls, label, levels }) => (
        <div key={cls} style={{ padding: '1rem', border: '1px solid #222', borderRadius: '8px', background: '#111' }}>
            ...
            <tbody>
              {levels.map(({ level, stats }) => (
                <tr key={level} style={{ borderBottom: '1px solid #1a1a1a' }}>
                  ...
                  <td style={{ padding: '0.3rem', textAlign: 'right', fontFamily: 'var(--font-geist-mono)' }}>
                    {stats.attack}
                  </td>
                  <td style={{ padding: '0.3rem', textAlign: 'right', fontFamily: 'var(--font-geist-mono)' }}>
                    {stats.defense}
          ...
          <div style={{ marginTop: '0.75rem' }}>
            <div style={{ fontSize: '0.7rem', opacity: 0.4, marginBottom: '0.35rem', textTransform: 'uppercase' }}>
              Spell Unlocks
            </div>
            {levels.map(({ level, spells }) => {
              const prevSpells =
                level === 1 ? [] : (levels.find((l) => l.level === LEVELS[LEVELS.indexOf(level) - 1])?.spells ?? []);

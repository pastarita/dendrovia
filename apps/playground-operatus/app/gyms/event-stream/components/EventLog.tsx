...
export function EventLog({ entries, onSelect }: { entries: EventEntry[]; onSelect: (entry: EventEntry) => void }) {
      ...
      >
        {entries.length === 0 ? (
          ...
          entries.map((entry) => {
            ...
            const preview = entry.payload != null ? JSON.stringify(entry.payload).slice(0, 80) : '';

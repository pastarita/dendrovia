...
export function MigrationTester({ mod, onMigrate }: { mod: OperatusMod; onMigrate: () => void }) {
  ...
  return (
    <div style={{ padding: '1rem 1.25rem', border: '1px solid #222', borderRadius: '8px' }}>
      <h3 style={{ fontSize: '0.95rem', fontWeight: 600, marginBottom: '0.75rem' }}>Migration Tester</h3>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1rem' }}>
        <button onClick={handleInjectV1} style={btnStyle}>
          Inject v1 Save
        </button>
        <button onClick={handleReload} style={btnStyle}>
          Reload Page
        </button>
        <button onClick={handleShowCurrent} style={btnStyle}>
          Show Current State
        </button>
      </div>

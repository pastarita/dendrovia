...
type ExhibitPayload =
  | { kind: 'pillar'; pillar: PillarInfo }
  | { kind: 'flow'; flow: DataFlow };
...
const FILTERS: MuseumFilter[] = [
  ...
  { id: 'pillars', label: 'Pillars', predicate: (item) => (item as MuseumExhibitDescriptor<ExhibitPayload>).data.kind === 'pillar' },
  { id: 'flows', label: 'Data Flows', predicate: (item) => (item as MuseumExhibitDescriptor<ExhibitPayload>).data.kind === 'flow' },
];
...
function renderDetail(item: MuseumExhibitDescriptor<ExhibitPayload>) {
  ...
  if (d.kind === 'pillar') {
    ...
    return (
      <div>
        <div style={{ padding: '0.75rem', border: `1px solid ${p.color}40`, borderRadius: 8, background: `${p.color}08`, marginBottom: '1rem' }}>
          <h3 style={{ margin: 0, fontSize: '1.1rem', color: p.color }}>{p.emoji} {p.name}</h3>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
          {relatedFlows.map((f, i) => {
            ...
            return (
              <div key={i} style={{ padding: '0.5rem 0.75rem', border: '1px solid #222', borderRadius: 4, fontSize: '0.8rem' }}>
...
function renderDetailFooter(item: MuseumExhibitDescriptor<ExhibitPayload>) {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', padding: '0.75rem', background: '#0a0a0a', borderRadius: 6, border: '1px solid #1a1a1a' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', justifyContent: 'center', fontSize: '0.75rem' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', justifyContent: 'center', fontSize: '0.75rem' }}>
          {PILLARS.slice(3).map((p) => (
            <span key={p.name} style={{ color: p.color, fontWeight: 600 }}>{p.emoji} {p.name}</span>
          ))}
        </div>
      </div>
    </div>

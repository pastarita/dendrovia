...
function getStatusEntries() {
  return (Object.entries(STATUS_COLORS) as [string, { fill: string }][]).map(
    ([key, val]) => ({ label: key, color: val.fill })
  );
}

function getDomainEntries() {
  return (Object.entries(PILLAR_COLORS) as [string, { fill: string }][]).map(
    ([key, val]) => ({ label: key, color: val.fill })
  );
}

function getFidelityEntries() {
  return (Object.entries(FIDELITY_COLORS) as [string, { fill: string }][]).map(
    ([key, val]) => ({ label: key, color: val.fill })
  );
}

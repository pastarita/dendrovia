...
import type { BoundaryContract, DendriteState } from '../types';
...
export function ContractDetailPanel({ store }: ContractDetailPanelProps) {
  ...
  const sourceEdge = diagram.edges.find((e) => `${e.source}->${e.target}` === selectedEdgeId);
  ...
  return (
    <div style={panelStyle}>
      ...
      <div style={{ fontWeight: 700, fontSize: '0.9rem' }}>
        {sourceNode?.label ?? sourceEdge.source} <span style={{ color: DT.accent }}>→</span>{' '}
        {targetNode?.label ?? sourceEdge.target}
      </div>

      {sourceEdge.label && <div style={{ color: DT.textMuted, fontStyle: 'italic' }}>{sourceEdge.label}</div>}

      {/* Types section */}
      {contract.types.length > 0 && (
        <div>
          <div style={sectionTitleStyle}>Types ({contract.types.length})</div>
          <div
            style={{
              display: 'flex',
              flexDirection: 'column',
      ...
      {contract.events.length > 0 && (
        <div>
          <div style={sectionTitleStyle}>Events ({contract.events.length})</div>
          <div
            style={{
              display: 'flex',
              flexDirection: 'column',
            ...
            {contract.events.map((e) => (
              ...
              >
                <span
                  style={{
                    fontSize: '0.7rem',
                    color: e.direction === 'emit' ? '#22C55E' : '#3B82F6',
                    fontWeight: 700,
                    flexShrink: 0,
                  }}
                >
                ...
                {e.payloadType && <span style={chipStyle}>{e.payloadType}</span>}
              </div>
            ))}
          </div>
        </div>
      ...
      {contract.schema && (
        <div>
          ...
          <div style={{ fontFamily: 'monospace', fontSize: '0.72rem' }}>{contract.schema}</div>
        </div>
      )}
    </div>
  );

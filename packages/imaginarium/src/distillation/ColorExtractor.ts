...
export function extractPalette(topology: CodeTopology): ProceduralPalette {
  try {
    ...
    // Base hue from dominant language
    const baseHue = getLanguageHue(dominantLang);

    // Saturation from complexity: low complexity = muted, high = vivid
    const saturation = Math.max(0.05, Math.min(0.2, 0.05 + avgComplexity * 0.01));

    // Lightness from churn: high churn = darker/moodier
    const lightness = Math.max(0.4, Math.min(0.7, 0.65 - avgChurn * 0.01));

    // Generate harmony
    const scheme: HarmonyScheme = files.length < 20 ? 'analogous' : 'split-complementary';

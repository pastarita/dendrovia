...
import { type AffinityTier, getDomainsForPillar, type PillarName, type RankedDomain } from './domain-registry';
...
function DomainCard({ domain, pillarHex }: { domain: RankedDomain; pillarHex: string }) {
  ...
  return (
    <a
      ...
      style={{
        ...
        border: isHero ? `2px solid ${pillarHex}55` : isFeatured ? '1px solid #333' : '1px solid #222',
      ...
      onMouseLeave={(e) => {
        e.currentTarget.style.borderColor = isHero ? `${pillarHex}55` : isFeatured ? '#333' : '#222';

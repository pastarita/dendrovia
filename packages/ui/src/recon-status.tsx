...
export function ReconStatusBar({ currentPillar }: { currentPillar: string }) {
  ...
  const current = data?.checkouts.find(
    (c) => c.name.toUpperCase() === currentPillar.toUpperCase()
  );
  ...
  if (!loading && !data) {
    return (
      ...
      >
        ...
        <div style={{ opacity: 0.5, marginBottom: "0.35rem" }}>
          No recon data
        </div>
        <button
          onClick={handleCopy}
  ...
  return (
    ...
    >
      ...
      >
        ...
        {current && (
          <>
            <span style={{ color: getStatusColor(current), fontSize: "0.6rem" }}>
              ●
            </span>
      ...
      {/* Shared alignment */}
      <div style={{ opacity: 0.5, fontSize: "0.65rem", marginBottom: "0.3rem" }}>
        shared: {data!.shared.alignment}
      </div>
      ...
      >
        ...
        <span style={{ fontWeight: 600, opacity: 0.8 }}>
          {data!.maturity.overall.toFixed(1)}
        </span>
      </div>
    </div>
  );
